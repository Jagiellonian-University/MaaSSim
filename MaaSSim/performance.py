...
def kpi_pax_pricing(*args,**kwargs):
    # calculate passenger indicators (global and individual)
    ...
    ret['TRAVEL_TIME'] = ret['ARRIVES_AT_DROPOFF']  # time in vehicle
    ret['WAIT_TIME'] = (ret['RECEIVES_OFFER'] + ret['MEETS_DRIVER_AT_PICKUP'] + ret.get('LOSES_PATIENCE', 0))
    ret['MATCHING_TIME'] = (ret['RECEIVES_OFFER'] + ret.get('LOSES_PATIENCE', 0))
    ret['PICKUP_TIME'] = (ret['MEETS_DRIVER_AT_PICKUP'])
    ret['OPERATIONS'] = ret['ACCEPTS_OFFER'] + ret['DEPARTS_FROM_PICKUP'] + ret['SETS_OFF_FOR_DEST']
    
    ret = ret[['TRAVEL_TIME', 'WAIT_TIME', 'MATCHING_TIME', 'PICKUP_TIME'] + [_.name for _ in travellerEvent]]
