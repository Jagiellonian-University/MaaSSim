...
def pool_price_fun(sim, veh, request, sp):
    ...
    logger = sim.logger.critical  # set what you want to see from the logger
    ...
    if len(request.rides) > 0:  # only if there is a choice
        ...
        if len(still_available_rides) > 1:  # only if we still have a choice
            ...
            if sp.get('probabilistic', False):
                ...
                # select by max profit for kpi 1, passengers with the highest fare are picked up first
                if kpi_type == 1:
                    print("Profit Maximization - Pick highest fare")
                    rf = still_available_rides[still_available_rides['indexes_orig'].map(len) > 1]
                    my_choice = rf[rf['fare'] == rf['fare'].max()].iloc[0]
                # select only solo rides for kpi 2
                elif kpi_type == 2:
                    print("Pooled rides")
                    rf = still_available_rides[(still_available_rides['indexes_orig'].map(len) == 1)]
                    my_choice = rf[rf['profit'] == rf['profit'].max()].iloc[0]
                # select by max profit on private rides for kpi 3
                elif kpi_type == 3:
                    print("Private rides - Pick highest profit")
                    rf = still_available_rides[(still_available_rides['indexes_orig'].map(len) == 1)]
                    my_choice = rf[rf['profit'] == rf['profit'].max()].iloc[0]
            ...
            # MAKE TWO OPTIONS OF CHOICE: DETERMINISTIC AND PROBABILISTIC:
